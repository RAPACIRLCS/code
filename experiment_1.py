...
# Construct equality constraints of IP
...
# Generate inequality constraints of IP
matrix_block_one = generate_constraint_matrix_block_one(
    value_powers, cost_powers, DEGREE
)
constraint_1_inner_product = np.sum(
    matrix_block_one[..., 1:] * mu_k[..., np.newaxis], axis=(0, 1)
)
inequality_constraint_1 = np.concatenate([np.array([-1]), constraint_1_inner_product])
inequality_constraint_2 = -1 * matrix_block_one
...
A_ub = np.concatenate(
    [A_ub, np.zeros((A_ub.shape[0], cost_weights.shape[0] + value_weights.shape[0]))],
    axis=1,
)
A_ub = np.concatenate(
    [
...
threshold = ((2 - GAMMA) / (1 - GAMMA)) * result.x[0]  # It works with eps = 0
...
true_p = np.zeros((RESOLUTION, RESOLUTION))
irl_p = np.zeros((RESOLUTION, RESOLUTION))
for i, s in enumerate(D_STATES):
    true_a = eval_policy(s, k_star)
    irl_a = irl_policy(s, D_STATES, irl_policy_sav)
    true_a_index = np.argmin(np.abs(D_ACTIONS - true_a))
    irl_a_index = np.argmin(np.abs(D_ACTIONS - irl_a))
    true_p[true_a_index, i] = 1
    irl_p[irl_a_index, i] = 1

plt.figure(figsize=(10, 5))
plt.subplot(1, 2, 1)
plt.imshow(true_p)
plt.title("Eval Policy")

plt.subplot(1, 2, 2)
plt.imshow(irl_p)
plt.title("IRL Policy")

plt.tight_layout()
plt.show()

plot_compare_cost_functions(
    cost, partial(cost_function, weights=cost_weights_x, powers=cost_powers)
)
